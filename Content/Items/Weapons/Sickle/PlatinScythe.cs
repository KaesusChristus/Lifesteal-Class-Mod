...
namespace LifeStealClass.Content.Items.Weapons.Sickle
{
    public class PlatinScythe : LifestealSickle
    {
            ...
            if (player.itemAnimation > 0)
            {
                ...
                if (tickTimer >= 60)
                {
                    ...
                    if (player.whoAmI == Main.myPlayer)
                    {
                        ...
                        Projectile.NewProjectile(player.GetSource_ItemUse(Item), player.Center, direction, ProjectileID.DiamondBolt, Item.damage, Item.knockBack, player.whoAmI);
                    }
                }
            }
            else
        ...
        public override void MeleeEffects(Player player, Rectangle hitbox)
        {
            ...
            {
                int dust = Dust.NewDust(hitbox.TopLeft(), hitbox.Width, hitbox.Height, DustID.PlatinumCoin);
